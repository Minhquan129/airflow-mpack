...
class AirflowWorker(Script):
	...
	def install(self, env):
		...
		Execute(format("{airflow_home}/airflow_env/bin/pip3.9 install  {service_packagedir}/scripts/all/* "))
		Execute(format("export SLUGIFY_USES_TEXT_UNIDECODE=yes && {airflow_home}/airflow_env/bin/pip3.9 install apache-airflow==2.3.3"))
		Execute(format("export SLUGIFY_USES_TEXT_UNIDECODE=yes && {airflow_home}/airflow_env/bin/pip3.9 install apache-airflow[celery]==2.3.3"))
